        ...
        if "courses" in data:
            for course in data["courses"]:
                if course["title"].find("-") != -1:
                    ...
                    courses.append(_)
                    courses.extend(_.split(" "))
        elif "filename" in data and "skills" in data["filename"] and data["filename"]["skills"]:
            for skill in data["filename"]["skills"]:
                courses.append(skill.lower())
                courses.extend(skill.lower().split(" "))
        elif "skills" in data:
            for skill in data["skills"]:
                if isinstance(skill, dict) and "name" in skill:
                    courses.append(skill["name"].lower())
                elif isinstance(skill, str):
                    courses.append(skill.lower())
                    
        courses = list(map(remove_punctuations, courses))
        if not courses:
            courses = ["programming", "software", "development"]
